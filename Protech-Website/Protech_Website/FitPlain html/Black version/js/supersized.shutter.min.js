...
0)return false;vars.thumb_page=vars.thumb_page+vars.thumb_interval;$(vars.thumb_list).stop().animate({"left":vars.thumb_page},{duration:500,easing:"easeOutExpo"})}}if(!siteStartOpen){for(var kk=0;kk<api.options.slides.length;kk++)$(api.options.slides[kk].main_title).hide();var self=this;self.ff=2;var thiEle=$(api.options.slides[vars.current_slide].main_title);$(api.options.slides[vars.current_slide].main_title).show();$(api.options.slides[vars.current_slide].main_title).children().each(function(){var sel=
$(this);sel.css({"top":Math.round($(window).height()-(thiEle.position().top-10)),"position":"relative"});self.ff=Number(self.ff)+1;sel.delay(self.ff*150).animate({"top":0},1500,"easeOutQuart",function(){if(sel.attr("data-gotoPage")!=undefined&&sel.data("slidNo")==vars.current_slide&&scrollPos<$(window).height()/2){if(sel.data("oneTime")==undefined)window.location.href="#"+sel.attr("data-gotoPage");if(sel.attr("data-oneTime")=="yes")sel.data("oneTime",true)}})})}siteStartOpen=true;for(var kk=0;kk<
api.options.slides.length;kk++)$(api.options.slides[kk].main_title).fadeOut(500);$(".supersized-nav li").find("a").removeClass("active");$(".supersized-nav li").eq(vars.current_slide).find("a").addClass("active");$(".supersized-nav").css({"margin-top":-vars.current_slide*30+"px"})},afterAnimation:function(){var self=this;self.ff=0;var curSlid=vars.current_slide;$(api.options.slides[vars.current_slide].main_title).show();var thiEle=$(api.options.slides[vars.current_slide].main_title);$(api.options.slides[vars.current_slide].main_title).children().each(function(){var sel=
$(this);sel.data("slidNo",curSlid);sel.css({"top":Math.round($(window).height()-(thiEle.position().top-10)),"position":"relative"});self.ff=Number(self.ff)+1;sel.delay(self.ff*150).animate({"top":0},1500,"easeOutQuart",function(){if(sel.attr("data-gotoPage")!=undefined&&sel.data("slidNo")==vars.current_slide&&scrollPos<$(window).height()/2){if(sel.data("oneTime")==undefined)window.location.href="#"+sel.attr("data-gotoPage");if(sel.attr("data-oneTime")=="yes")sel.data("oneTime",true)}})});if(api.options.progress_bar&&
!vars.is_paused)theme.progressBar()},progressBar:function(){$(vars.progress_bar).stop().css({left:-$(window).width()}).animate({left:0},api.options.slide_interval)},min_thumb:function(){}};$.supersized.themeVars={progress_delay:false,thumb_page:false,thumb_interval:false,image_path:"images/supersized/",play_button:"#pauseplay",next_slide:"#nextslide",prev_slide:"#prevslide",next_thumb:"#nextthumb",prev_thumb:"#prevthumb",slide_caption:"#slidecaption",slide_current:".slidenumber",slide_total:".totalslides",
slide_list:"#slide-list",thumb_tray:"#thumb-tray",thumb_list:"#thumb-list",thumb_forward:"#thumb-forward",thumb_back:"#thumb-back",tray_arrow:"#tray-arrow",tray_button:"#tray-button",progress_bar:"#progress-bar"};$.supersized.themeOptions={progress_bar:1,mouse_scrub:0}})(jQuery);
