...
1){var setIntr=base.options.slides[vars.current_slide].slide_interval!=undefined?base.options.slides[vars.current_slide].slide_interval:base.options.slide_interval;vars.slideshow_interval=setInterval(base.nextSlide,setIntr)}else vars.is_paused=true;$(".load-item img").bind("contextmenu mousedown",function(){return false})}$(window).resize(function(){base.resizeNow()})};base.resizeNow=function(){return base.$el.each(function(){$("img",base.el).each(function(){thisSlide=$(this);var ratio=(thisSlide.data("origHeight")/
...
if(vars.is_paused){vars.is_paused=false;if(typeof theme!="undefined"&&typeof theme.playToggle=="function")theme.playToggle("play");var setIntr=base.options.slides[vars.current_slide].slide_interval!=undefined?base.options.slides[vars.current_slide].slide_interval:base.options.slide_interval;if(!$.supersized.vars.firstLoad){$.supersized.vars.firstLoad=true;vars.slideshow_interval=setInterval(base.nextSlide,base.options.slides[vars.current_slide].slide_interval)}else vars.slideshow_interval=setInterval(base.nextSlide,
...
if(vars.current_slide==totalSlides-targetSlide){if(!vars.is_paused){var setIntr=base.options.slides[vars.current_slide].slide_interval!=undefined?base.options.slides[vars.current_slide].slide_interval:base.options.slide_interval;vars.slideshow_interval=setInterval(base.nextSlide,setIntr)}return false}if(totalSlides-targetSlide>vars.current_slide){vars.current_slide=totalSlides-targetSlide-1;vars.update_images="next";base._placeSlide(vars.update_images)}else if(totalSlides-targetSlide<vars.current_slide){vars.current_slide=
...
undefined?base.options.slides[vars.current_slide].slide_interval:base.options.slide_interval;vars.slideshow_interval=setInterval(base.nextSlide,setIntr);if(base.options.stop_loop&&vars.current_slide==base.options.slides.length-1)base.playToggle()}if(typeof theme!="undefined"&&typeof theme.afterAnimation=="function")theme.afterAnimation();return false};base.getField=function(field){return base.options.slides[vars.current_slide][field]};base.init()};$.supersized.vars={thumb_tray:"#thumb-tray",thumb_list:"#thumb-list",
