	...
	theme = {
	 	...
	 	beforeAnimation : function(direction){
			...
			if(!siteStartOpen){
				for(var kk=0; kk< api.options.slides.length; kk++){
					$(api.options.slides[kk].main_title).hide();
				}
				var self = this;
				self.ff = 2;
				var thiEle = $(api.options.slides[vars.current_slide].main_title);
				...
				$(api.options.slides[vars.current_slide].main_title).children().each(function(){
					var sel = $(this);
					sel.css({"top": Math.round($(window).height()-(thiEle.position().top-10)), "position":"relative"});
					self.ff = Number(self.ff)+1;
					sel.delay(self.ff*150 ).animate({ "top":0},1500,"easeOutQuart", function(){
						if(sel.attr("data-gotoPage") != undefined && sel.data("slidNo") == vars.current_slide && scrollPos < $(window).height()/2){							
							if(sel.data("oneTime") == undefined){ window.location.href = "#"+sel.attr("data-gotoPage"); }							
							if(sel.attr("data-oneTime") == "yes"){	sel.data("oneTime", true);	}
						}
					});													
				});			
			}
			...
			for(var kk=0; kk< api.options.slides.length; kk++){
				$(api.options.slides[kk].main_title).fadeOut(500);
			}
		...
		----------------------------*/
	 	afterAnimation : function(){
			...
			var self = this;
				self.ff = 0;
				var curSlid = vars.current_slide;
				$(api.options.slides[vars.current_slide].main_title).show();
				var thiEle = $(api.options.slides[vars.current_slide].main_title);
				$(api.options.slides[vars.current_slide].main_title).children().each(function(){
					var sel = $(this);
